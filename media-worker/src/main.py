...
def main():
    ...
    if Config.IS_MOCK_MODE:
        ...
        expected = ["downloading", "extracting", "analyzing", "aggregating", "generating_evidence", "completed"]
        print(f"\n[MOCK] Status sync verification:")
        print(f"  Expected {len(expected)} status updates, received {len(received)}")
        for exp, rec in zip(expected, received):
            rec_status = rec.get("status", "?")
            ok = "✓" if rec_status == exp else "✗"
            print(f"  {ok} {exp} → received: {rec_status}")
